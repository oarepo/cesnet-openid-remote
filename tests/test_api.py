...
def test_add_group_to_role(communities_app, cesnet_groups, community, protected_roles):
    """Test adding a cesnet group to invenio role."""
    roles = Role.query.all()
    assert len(roles) == 4
    assert len(CesnetGroup.query.all()) == 2

    group = cesnet_groups[0]
    ...
    with pytest.raises(OAuthCESNETRoleProtected):
        current_cesnet_openid.add_group_role(group, protected_roles[0])


def test_rm_role_from_group(communities_app, cesnet_groups, community, protected_roles):
